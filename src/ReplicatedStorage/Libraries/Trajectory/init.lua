-- Written by EgoMoose (Public library), modified for my use
...
function Trajectory:CalculateSingle(x0, v0, ignoreList)
	...
	repeat
		...
		local rayResult = Raycast.new(ignoreList, "Blacklist", origin, direction.Unit, direction.Magnitude)
		if not rayResult then continue end
		hit, pos, normal, material = rayResult.Instance, rayResult.Position, rayResult.Normal, rayResult.Material
